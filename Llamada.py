...
class Llamada:
    ...
    def __init__(self, conexion):
        self.conn = conexion.establecer_conexion_llamada(ready_handler=self.ready_cb)
    ...
    def src_pad_added (self, stream, pad, codec):
        tipo = stream.get_property ("media-type")
        if tipo == farsight.MEDIA_TYPE_AUDIO:
            sink = gst.parse_bin_from_description("audioconvert ! audioresample ! audioconvert ! autoaudiosink", True)
        elif tipo == farsight.MEDIA_TYPE_VIDEO:
...
class LlamadaSaliente(Llamada):
    def __init__(self, conexion, contact):
        Llamada.__init__(self, conexion)
...
#        Request a channel
    ...
    def ready_cb(self, conn):
        print "estoy en el ready_cb de la Llamada Saliente"
        handle = self.conn[CONN_INTERFACE].RequestHandles(
            CONNECTION_HANDLE_TYPE_CONTACT, [self.contact])[0]
        self.handle = handle

        if CONN_INTERFACE_CAPABILITIES in self.conn.get_valid_interfaces():
            self.conn[CONN_INTERFACE_CAPABILITIES].connect_to_signal('CapabilitiesChanged', self.capabilities_changed_cb)
            self.got_handle_capabilities(self.conn[CONN_INTERFACE_CAPABILITIES].GetCapabilities([handle]))
        else:
    ...
    def channel_ready_cb(self, channel):
        Llamada.channel_ready_cb(self, channel)
